...
####ORIGINAL CACHE RENDERING FROM POINT CLOUD AND DEPTH####
        ## path al PLY che hai esportato da Blender
        #ply_path = "cache_pointcloud.ply"
#
        #pcd = o3d.io.read_point_cloud(ply_path)
        #pts_np = np.asarray(pcd.points, dtype=np.float32)      # [N,3]
        ## se hai colori nel PLY, Open3D li porta in [0..1]:
        #if pcd.has_colors():
        #    cols_np = (np.asarray(pcd.colors) * 255).astype(np.uint8)  # [N,3]
        #else:
        #    cols_np = None
        #
        #B, F, V, H, W = 1, 1, 1, 1, 1
...
#
        ...
        #    generator=generator,
        #    input_depth=None,
        #    # serve comunque un'immagine di riferimento per il warping:
        #    input_image=moge_image_b1chw_float[:, 0].clone(),  
        #    input_w2c=moge_image_b1chw_float[:, 0].clone(),
        #    input_intrinsics=moge_intrinsics_b133[:, 0],
        #    input_points=input_points,      # LA TUA point-cloud importata
        ...
        # 2) Costruisci input_points con H=W=1
        B, F, V = 1, 1, 1
        N = pts_np.shape[0]
        input_points = (
            torch.from_numpy(pts_np)
        ...
        # 3) Costruisci input_image “point‐wise” con H=W=1
        if cols_np is not None:
            img = torch.from_numpy(cols_np).float() / 255.0  # (N,3)
            # [B,F,N,V,C=3,H=1,W=1]
            input_image = img.view(B, F, N, V, 3, 1, 1).to(device)
        else:
            # placeholder nero se non hai colori
            input_image = torch.zeros(B, F, N, V, 3, 1, 1, device=device)


        # ─── 1) Proiezione Z‐Buffer ───────────────────────────────────────────────────
        # pts_np: [N,3], cols_np: [N,3], intrinsics e w2c già caricati
        H, W = args.height, args.width
        # inizializza
        Z_buf = np.full((H, W), np.inf, dtype=np.float32)
        Img   = np.zeros((H, W, 3), dtype=np.uint8)

        # per ogni punto della cloud
        for k in range(pts_np.shape[0]):
            # punti in coordinate camera
            X_cam = (moge_initial_w2c_b144[0,0].cpu().numpy() @ np.append(pts_np[k], 1.0))[:3]
            if X_cam[2] <= 0: 
                continue  # dietro la camera
            # pixel
            u = int((moge_intrinsics_b133[0,0,0,0].cpu()*X_cam[0] / X_cam[2]) + moge_intrinsics_b133[0,0,0,2].cpu())
            v = int((moge_intrinsics_b133[0,0,1,1].cpu()*X_cam[1] / X_cam[2]) + moge_intrinsics_b133[0,0,1,2].cpu())
            if 0 <= u < W and 0 <= v < H and X_cam[2] < Z_buf[v,u]:
                Z_buf[v,u]   = X_cam[2]
                Img[v,u]     = cols_np[k] if cols_np is not None else [0,0,0]

        # normalizza per la cache
        input_image = (torch.tensor(Img,     dtype=torch.float32)[None, None].permute(0,1,4,2,3) / 127.5 - 1.0)
        ...
        ##### FROM POINT CLOUD AND DEPTH STRANGE IMAGE #####
        #cache = Cache3D_Buffer(
        #    frame_buffer_max=frame_buffer_max,
        #    noise_aug_strength=args.noise_aug_strength,
        #    generator=generator,
        #    input_format=["B","F","N","V","C","H","W"],
...
#### FROM SINGLE IMAGE AND DEPTH ####
        ...
        ### WITHOUT MEMORY OPT####
        #rendered_warp_images, rendered_warp_masks = cache.render_with_pseudo_image(
        #generated_w2cs[:, 0:sample_n_frames],
        #generated_intrinsics[:, 0:sample_n_frames],
        #render_depth=False
        #)
        # ↑ Qui inizia il nuovo batching a chunk di frame
        all_imgs, all_masks = [], []
        chunk_size = 16  # numero di frame per sotto‐batch (puoi sperimentare: 8, 12, 16, ...)
        total = generated_w2cs.shape[1]

        for start in range(0, total, chunk_size):
            end = min(start + chunk_size, total)
            imgs_chunk, masks_chunk = cache.render_cache(
                generated_w2cs[:, start:end],
                generated_intrinsics[:, start:end],
                #render_depth=False
            )
            all_imgs.append(imgs_chunk)
            all_masks.append(masks_chunk)

        # Ricompone i chunk in un unico tensor su dimensione frame (dim=1)
        rendered_warp_images = torch.cat(all_imgs, dim=1)
        rendered_warp_masks  = torch.cat(all_masks,  dim=1)
        print("rendered_warp_images shape:", rendered_warp_images.shape)
        print("rendered_warp_masks shape:", rendered_warp_masks.shape)
        ...
        for num_iter in range(1, num_ar_iterations):
            ...
            #current_segment_w2cs,
            #current_segment_intrinsics,
            #render_depth=False,    # oppure True se ti serve anche la depth
            #pseudo_h=1             # usa 1 salvo che tu voglia un’altra altezza
            #)
