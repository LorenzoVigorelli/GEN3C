...
# /export/scratch/lvigorel/GEN3C/NEWSHOEFULL.ply
# /export/scratch/lvigorel/GEN3C/shoe_points.ply
# /export/scratch/lvigorel/GEN3C/shoehalf.ply

        # ─── 1) Carica il PLY correttamente ───
        ...
        if pcd.has_colors():
            ...
            # CORREZIONE: controlla se Open3D ha normalizzato automaticamente
            if colors_raw.max() <= 1.0 and np.all(colors_raw >= 0.99):
                # Caso problematico: tutti i colori sono ~1.0 (Open3D ha normalizzato male)
                print("⚠️ Open3D ha normalizzato i colori - carico manualmente")

                # Carica manualmente i colori dal PLY binario
                import struct
                with open(ply_path, 'rb') as f:
                    # Salta header fino a 'end_header'
                    while True:
                    ...
                    # Leggi dati binari: 3 double (x,y,z) + 3 uchar (r,g,b)
                    colors_manual = []
                    struct_format = '<3d3B'  # little endian, 3 double, 3 unsigned char
                    ...
                    for i in range(vertex_count):
                        ...
                        # unpacked = (x, y, z, r, g, b)
                        r, g, b = unpacked[3], unpacked[4], unpacked[5]
                        colors_manual.append([r, g, b])

                cols_np = np.array(colors_manual, dtype=np.uint8)
                print(f"Colori caricati manualmente: {len(colors_manual)}")

            elif colors_raw.max() <= 1.0:
                # Colori normalizzati correttamente [0-1], moltiplica per 255
                cols_np = (colors_raw * 255).astype(np.uint8)
                print("Colori convertiti da [0-1] a [0-255]")
            else:
                # Colori già in [0-255]
                cols_np = colors_raw.astype(np.uint8)
                print("Colori già in formato [0-255]")
        else:
            cols_np = None
            print("Nessun colore trovato nel PLY")

        if cols_np is not None:
            print(f"Final cols_np range: [{cols_np.min()}, {cols_np.max()}]")
            print(f"Colori unici trovati: {len(np.unique(cols_np.reshape(-1, 3), axis=0))}")

        # ─── 2) Costruisci input_points ───
        B, F, V = 1, 1, 1
        N = pts_np.shape[0]
        input_points = (
            torch.from_numpy(pts_np)
            .view(B, F, N, V, 1, 1, 3)  # [1,1,N,1,1,1,3]
            .to(device)
        )

        # ─── 3) Costruisci input_image ───
        if cols_np is not None:
            img = torch.from_numpy(cols_np).float() / 255.0  # (N,3) normalizzato [0,1]
            input_image = img.view(B, F, N, V, 3, 1, 1).to(device)  # [B,F,N,V,C=3,H=1,W=1]
            print(f"input_image range: [{input_image.min():.6f}, {input_image.max():.6f}]")
        else:
            input_image = torch.zeros(B, F, N, V, 3, 1, 1, device=device)
            print("Usando colori neri (placeholder)")

        print("Starting Z Buffer projection")
        H, W = args.height, args.width

        # 1) Prepara point-cloud e colori
        # pts_np: [N,3], cols_np: [N,3] uint8 0–255
        pts = torch.from_numpy(pts_np).float().to(device)                  # [N,3]
        cols = torch.from_numpy(cols_np).float().to(device) / 255.0        # [N,3] in [0,1]
        cloud = Pointclouds(points=[pts], features=[cols])

        # 2) Costruisci la camera PyTorch3D da moge_initial_w2c_b144 e moge_intrinsics_b133
        # Estrai extrinsics MoGe: world→camera → inverti per camera→world
        w2c = moge_initial_w2c_b144[0, 0]      # [4,4]
        c2w = torch.inverse(w2c)
        R = c2w[:3, :3][None]                  # [1,3,3]
        T = c2w[:3, 3][None]                   # [1,3]

        # Intrinseche MoGe
        K = moge_intrinsics_b133[0, 0]         # [3,3]
        ...
        # Normalizza cx, cy in range [-1,1] (coordinate immagine normalizzate)
        px = (cx / W - 0.5) * 2     # cx in [-1, 1]
        py = (cy / H - 0.5) * 2     # cy in [-1, 1]

        # Camera con fx, fy, cx, cy espliciti
        cameras = PerspectiveCameras(
            device=device,
            R=R,
            T=T,
            focal_length=((fx, fy),),              # focali in pixel
            principal_point=((px, py),),           # normalizzati in [-1,1]
            image_size=((H, W),)                   # dimensione immagine
        )

        # 3) Impostazioni di rasterizzazione e compositing
        rast_settings = PointsRasterizationSettings(
            image_size=(H, W),
            radius=0.005,                # raggio di splatting in unità di scena (aggiusta a piacere)
            points_per_pixel=1,          # quante depth tenere per pixel
        )
        rasterizer = PointsRasterizer(cameras=cameras, raster_settings=rast_settings)
        compositor = AlphaCompositor()
        renderer = PointsRenderer(rasterizer=rasterizer, compositor=compositor)

        # 4) Renderizza: ottieni un tensor [1,H,W,4] RGBA
        rgba = renderer(cloud)            # valore in [0,1]
        rgb = (rgba[0, ..., :3] * 255).byte().cpu().numpy()   # [H,W,3] uint8
        depth = renderer.rasterizer(cloud).zbuf[0, ..., 0].cpu().numpy()  # [H,W] primo layer depth

        # 5) Trasforma nei formati attesi da Cache3D_Buffer
        #   input_image: [B=1, F=1, C=3, H, W] in [-1,1]
        input_image = ((torch.from_numpy(rgb).permute(2,0,1).float() / 127.5) - 1.0)[None,None].to(device)
        #   input_depth: [1,1,H,W]
        input_depth = torch.from_numpy(depth)[None,None].float().to(device)
        img = ((input_image[0,0].clamp(-1,1) + 1) * 127.5).byte().permute(1,2,0).cpu().numpy()
        img_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        cv2.imwrite("image_opencv.png", img_bgr)

        print("Starting video generation")
        # ─── 2) Crea la cache con immagine + depth ─────────────────────────────────
        cache = Cache3D_Buffer(
            frame_buffer_max    = frame_buffer_max,
            noise_aug_strength  = args.noise_aug_strength,
            generator           = generator,
            input_format=None,
            input_image         = input_image.squeeze(1).to(device), # [B, F, N, V, C, H, W], range [-1,1]
...
##### EXPORT CACHE POINT CLOUD TO PLY FOR BLENDER ######
        #
        #cache = Cache3D_Buffer(
        #    frame_buffer_max=frame_buffer_max,
        #    generator=generator,
        ...
        ## 1) Preleva i tensori dalla cache (qui assumiamo B=1, F=1, N=1, V=1)
        #pts = cache.input_points        # shape [B, F, N, V, H, W, 3]
        #cols = cache.input_image        # shape [B, F, N, V, C, H, W], C=3
#
        ## 2) Squeeze e reshape in array (N = H*W)
        #pts = pts[0,0,0,0]              # → [H, W, 3]
        #cols = cols[0,0,0,0]            # → [3, H, W]
#
        #H, W = pts.shape[:2]
        #pts = pts.reshape(-1, 3).cpu().numpy()   # [H*W, 3]
        ## Il range di cols è [-1,1], ricampioniamo a [0,1] o [0,255]
        #cols = ((cols.permute(1,2,0).cpu().numpy() * 0.5) + 0.5)  # → [H, W, 3] in [0,1]
        #cols = cols.reshape(-1, 3)
#
        ## 3) Costruisci e salva la point cloud PLY
        #pcd = o3d.geometry.PointCloud()
        #pcd.points = o3d.utility.Vector3dVector(pts)
        #pcd.colors = o3d.utility.Vector3dVector(cols)
        #o3d.io.write_point_cloud("cache_pointcloud.ply", pcd)
        #print("Esportato cache_pointcloud.ply per Blender")

# https://chatgpt.com/share/e/6866953d-1644-800b-8f5b-0dce438fa673